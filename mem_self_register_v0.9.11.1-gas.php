...
function mem_self_register_form_submit()
{
	...
	$mem_profile['email'] = $email = $mem_form_values['email'];
        /* 0.9.11.1-gas the line below has been modified to permit registration with
        e-mail only. If username is not set within the form, a random value is generated.*/
        if ( !isset($mem_form_values['name'])) {
          $mem_form_values['name'] = uniqid( 'user', false ) ;
        }
        $mem_profile['name'] = $username = $mem_form_values['name']; //original
        //if (isset($mem_form_values['name']) ? $mem_profile['name'] = $username = $mem_form_values['name'] : $mem_profile['name'] = $username = 'user'.uniqid( rand(), true ) ); //modified
	...
	if (safe_row('user_id', mem_get_user_table_name(), "name = '".doSlash($username)."'")) {
        ...
        /* 0.9.11.1-gas this one check that the e-mail submitted is not alredy registered.
        comment out if you don't need it, as textpattern by default admit multiple
...
This plugin is licensed under the <a href="http://www.fsf.org/licensing/licenses/info/GPLv2.html" rel="nofollow">GPLv2</a>.

h2. Installation

This plugin requires the plugin mem_form <a href="./index.php?event=plugin&amp;step=plugin_help&amp;name=mem_form" rel="nofollow">(help)</a>.
...
* @<txp:mem_name />@ &#8212; The name submitted by the user.
* @<txp:mem_username />@ &#8212; The username submitted by the user.
* @<txp:mem_email />@ &#8212; The email address submitted by the user.
* @<txp:mem_password />@ &#8212; The password generated for the user.
* @<txp:mem_sitename />@ &#8212; The site name as specified in the Site Configuration (prefs tab).
* @<txp:mem_siteurl />@ &#8212; The url to the front page of this site.
* @<txp:mem_loginurl />@ &#8212; The url to the Textpattern log in page.
* @<txp:mem_admin_name />@ &#8212; The name of the administrative contact. This will automatically be in the email from field.
* @<txp:mem_admin_email />@ &#8212; The email address of the administrative contact. This will automatically be in the email from field.


Starter Template:

bc.. Dear <txp:mem_name />,
...
Site <span class="caps">URL</span>: <txp:mem_siteurl />
Login <span class="caps">URL</span>: <txp:mem_loginurl />
...
bc.. <txp:mem_self_register_form from="admin@mysite.com" subject="[MySite] Your account info" thanks_form="registered_form">
	<fieldset>
		<legend>Register</legend>
		<txp:mem_form_text name="RealName" label="Full Name" /><br />
		<txp:mem_form_text name="name" label="Username" /><br />
...
h3. if_self_registered

This will output the contents of the tag if the user has already been registered. To work properly, this requires browser cookies after the initial form submission. This tag supports @<txp:else />@.

Example:

bc.. <txp:if_self_registered>
	You already have an account.
<txp:else />
	...registration form...
</txp:if_self_registered>

h3(#mem_change_password_form). mem_self_change_password_form
...
* @form@ <span>string</span>: Specifies which form contains the HTML form template.
* @email_form@ <span>string</span>: Specifies which form contains the email message template that will be sent to a user upon registering.
* @from@ <span>string</span>: Registration email's From address. Defaults to mem_self_admin_email preference.
* @reply@ <span>string</span>: Registration email's Reply To address.
* @subject@ <span>string</span>: Subject for email. Default is "[SITENAME] Password Changed"
...
* <strong>var</strong> &#8212; Specifies the profile value name to output. Supported values are "user_id", "user", "RealName", "email", and any other db field.
* <strong>form</strong> &#8212; A form containing other tags to parse.
* <strong>userid</strong> &#8212; If specified, the profile information for the given user_id will be fetched.
* <strong>user</strong> &#8212; If specified, the profile information for the user with the given name will be fetched.

Example for non logged in user

bc.. <txp:mem_profile user="jdoe">
...
bc. Welcome back <txp:mem_profile var="RealName" />
...
* <strong>form</strong> &#8212; The form containing the html form requesting username and email. If not specified, tag contents are used.
* <strong>form_mail</strong> &#8212; The form used for the confirmation email's message.
* <strong>subject</strong> &#8212; The confirmation email's subject.
* <strong>from</strong> &#8212; Email from header
* <strong>reply</strong> &#8212; Email reply to header.
* <strong>confirm_url</strong> &#8212; <span class="caps">URL</span> that links back to the password reset form (this tag).
* <strong>new_subject</strong> &#8212; The new password email's subject.
* <strong>new_form_mail</strong> &#8212; The form used for the new password email's message.
* <strong>check_name</strong> &#8212; Set to "0" if the form does not contain a username field.
* <strong>check_email</strong> &#8212; Set to "0" if the form does not contain an email field.

Starter Template

bc.. <txp:mem_self_password_reset_form form_mail="reset_password_form" new_form_mail="new_password_email">
	<txp:mem_form_text name="name" label="Username:" /><br />
...
* @user_levels@ <span>string</span>: Comma separated list of user levels that should be included in the count. Default is all "0,1,2,3,4,5,6"
* @wraptag@ <span>string</span>: HTML tag to wrap around the result.
* @class@ <span>string</span>: CSS class name for wraptag.

h2. Changelog

h3. 0.9.11.1-gas

* Added check for unique e-mail while registering new user.
* If there is not a username field in the register form, a random username is generated.
** This is also a bug-fix: before this, a new user with blank username field would have been registered.
